...
function generateContracts(){
	...
	deBugLimitContracts = [1,2,3];
	
	//contract 1 
	difficulty = enemyDifficulty.easy;
	dungeonType = randomDungeonType([1,3,4]);
	contractType = randomContractType(dungeonType, 0.3, 0.5, deBugLimitContracts);
	reward = 100 + round(random_range(20, 70));
...
function createContract(_difficulty, _dungeonType, _contractType, _reward, _description,_mapInfo, _extra = -1) constructor{
...
function randomDungeonType(_limitation = -1){
	randomize();
	maxRoll = 4;
...
function randomContractType(_dungeonType, _exploreMin = 0.5,_exploreMax = 0.8, _limitation = -1){
	randomize();
	maxRoll = 3;
	contractType = [];
...
function randomMapInfo(_difficulty, _contractType){
	mapInfo = [];
	switch(_difficulty){
...
function randomDescription(_dungeonType, _contractType){
	description = [];
